...
async function startScreenShare() {
    try {
        ...
        // Replace the user's video stream with the screen stream
        const videoTrack = screenStream.getVideoTracks()[0];
        videoTrack.onended = () => stopScreenShare();

        // Send the screen stream to other participants
        const sender = peers[Object.keys(peers)[0]].peerConnection.getSenders().find(s => s.track.kind === 'video');
...
// Listen for when another user starts screen sharing
socket.on("screen-sharing-started", (username) => {
    alert(`${username} is now sharing their screen.`);
});

socket.emit("screen-sharing-started", myname);
